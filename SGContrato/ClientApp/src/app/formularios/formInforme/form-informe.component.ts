...
export class FormInformeComponent {
  ...
  constructor(private http: HttpClient, public activeModal: NgbActiveModal, private formBuilder: FormBuilder, private fb: FormBuilder, public cs: ContratosService, public is: InformesService) {
    this.createForm();
    this.is.getTipoInforme('5').subscribe(tiposDesdeWS => this.tiposInforme = tiposDesdeWS, error => console.error(error));
  }
